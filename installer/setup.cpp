...
namespace{
bool HasSwitch(const wchar_t* value){
 int argc=0;auto argv=CommandLineToArgvW(GetCommandLineW(),&argc);bool found=false;
 for(int i=1;argv&&i<argc;i++)if(!_wcsicmp(argv[i],value)){found=true;break;}
 if(argv)LocalFree(argv);return found;
}
bool Silent(){return HasSwitch(L"/silent")||HasSwitch(L"/verysilent")||HasSwitch(L"/quiet");}
...
void RegisterUninstaller(const std::wstring& dir){
 ...
 const std::wstring icon=dir+L"\\VetroLook.exe";
 RegSetKeyValueW(HKEY_CURRENT_USER,key.c_str(),L"DisplayName",REG_SZ,L"VetroLook",sizeof(L"VetroLook"));
 RegSetKeyValueW(HKEY_CURRENT_USER,key.c_str(),L"DisplayVersion",REG_SZ,L"1.0.1",sizeof(L"1.0.1"));
 ...
 const std::wstring quietUninstaller=uninstaller+L" /silent";
 RegSetKeyValueW(HKEY_CURRENT_USER,key.c_str(),L"QuietUninstallString",REG_SZ,quietUninstaller.c_str(),DWORD((quietUninstaller.size()+1)*sizeof(wchar_t)));
 RegSetKeyValueW(HKEY_CURRENT_USER,key.c_str(),L"DisplayIcon",REG_SZ,icon.c_str(),DWORD((icon.size()+1)*sizeof(wchar_t)));
 ...
 DWORD disabled=1;RegSetKeyValueW(HKEY_CURRENT_USER,key.c_str(),L"NoModify",REG_DWORD,&disabled,sizeof(disabled));RegSetKeyValueW(HKEY_CURRENT_USER,key.c_str(),L"NoRepair",REG_DWORD,&disabled,sizeof(disabled));
}
...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,LPWSTR,int){
 ...
 auto dir=InstallDir();
 if(dir.empty()){if(!silent)MessageBoxW(nullptr,L"Could not resolve the local install folder.",L"VetroLook Setup",MB_OK|MB_ICONERROR);return 1;}
 ...
 if(!WritePayload(targetExe,error)){
  if(!silent)MessageBoxW(nullptr,error.c_str(),L"VetroLook Setup",MB_OK|MB_ICONERROR);
  return 1;
 }
